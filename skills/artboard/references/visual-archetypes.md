...
- **A mascot, where one is used, becomes the reading pointer.** Not every archetype
  accepts one. `references/mascots.md` carries the compatibility table, the three roles,
  and the counts. The short version: Character Flowchart, Pipeline Stages and Annotated
  Blueprint are native; Specimen Grid, Cheat Sheet Poster, Terminal Card, Logo Grid and
  Spec Sheet take none.
...
| Pipeline Stages vs Mascot Flow | Mascot Flow | if the stages happen in a fixed order a character can walk |
