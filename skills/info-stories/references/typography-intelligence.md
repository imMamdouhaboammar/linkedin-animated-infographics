...
- `font_policy` is either `exact-required` (only when the source or an embedded licensed font provides the family) or `fallback-accepted`; screenshot inspection never proves an exact font.
