...
Version 3.4.0 gives Codex and ChatGPT a capability-negotiated OpenAI autopilot distribution while preserving the existing Claude execution path.
...
The 3.4.0 work does not replace Claude agents with OpenAI role passes. Claude remains on `native-worker-graph`. Only release version metadata is synchronized where repository parity requires it.
...
Version 3.4.0 also adds real project-scoped Codex adapters for canonical product workers:
...
Version 3.4.0 is prepared as a skills-only OpenAI update. The tracked handoff lives under `submission/` and includes listing metadata plus exactly five positive and three negative reviewer cases.
