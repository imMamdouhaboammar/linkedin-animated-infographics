...
5. When repository demos are available, use `tools/demo_taste.py` to select at most three relevant examples. Treat them as inspiration evidence only. Assign each demo one bounded inspiration job and transfer rules through `Evidence -> Observation -> Transferable Rule -> Anti-Rule`.
