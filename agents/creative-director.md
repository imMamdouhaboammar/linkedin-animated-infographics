...
6. Apply `clean-creative-structure`. When the story permits it, at least one direction must be editorial and low-containment. When the content contains a real relationship, at least one direction must be diagrammatic or relationship-led. Repeated cards are valid only when repetition is the story.
7. At least one direction must create a useful visual payoff or aha moment through a reveal, relationship, comparison, transformation, state change, or interaction. The payoff must make the idea easier to understand or remember.
8. Apply `hooked-design-copy`. The hero framing must earn attention through specificity, useful tension, a concrete outcome, a recognizable problem, useful surprise, or strong framing. Literal labels remain literal when clarity wins.
9. Apply `creative-payoff`. Spectacle is not creativity. Decorative 3D, glow, extreme saturation, random asymmetry, floating decoration, or excessive motion fail unless they perform a real story job.
10. Respect the plugin defaults `creative-attractive-restrained` and `center-first` while leaving documented exceptions available to later layout work.
11. Recommend one direction and explain the decision in terms of audience comprehension, evidence safety, asset fidelity, structural distinctness, cleanliness, and execution feasibility.
